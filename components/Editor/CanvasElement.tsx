...
const CanvasElement: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const videoLayer = layer as ImageLayer;
    const baseVolume = videoLayer.volume ?? 1;
    const fadeInSec = (videoLayer.audioFadeInMs ?? 0) / 1000;
    const fadeOutSec = (videoLayer.audioFadeOutMs ?? 0) / 1000;

    const applyVolumeEnvelope = () => {
      const durationSec = videoLayer.duration ?? videoElement.duration ?? 0;
      const currentSec = videoElement.currentTime ?? 0;

      let fadeFactor = 1;
      if (fadeInSec > 0) {
        fadeFactor = Math.min(fadeFactor, Math.max(0, Math.min(1, currentSec / fadeInSec)));
      }
      if (fadeOutSec > 0 && durationSec > 0) {
        fadeFactor = Math.min(
          fadeFactor,
          Math.max(0, Math.min(1, (durationSec - currentSec) / fadeOutSec))
        );
      }

      const effectiveVolume = Math.max(0, Math.min(1, baseVolume * fadeFactor));
      videoElement.volume = effectiveVolume;
      videoElement.muted = effectiveVolume === 0;
    };
