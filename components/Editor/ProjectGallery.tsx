...
interface Props {
  ...
  onNewProject: () => void;
  useLocalProjects?: boolean;
}

const ProjectGallery: React.FC<Props> = ({ onClose, onLoadProject, onNewProject, useLocalProjects = false }) => {
  ...
  const loadProjects = async () => {
    try {
      if (useLocalProjects) {
        setProjects(localProjectStore.toMetadata(localProjectStore.list()));
        return;
      }
  ...
  const confirmDelete = async () => {
    ...
    if (useLocalProjects) {
      localProjectStore.delete(projectToDelete);
      loadProjects();
