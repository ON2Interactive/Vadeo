...
import {
  Play, Pause, RotateCcw, Volume2, VolumeX, Layout,
  AlignLeft, AlignCenter, AlignRight, Type as TypeIcon,
  Plus, Minus, ChevronDown, Check,
  Video as VideoIcon, MoveRight, Clock, Zap, Sparkles, Wand2
} from 'lucide-react';
            ...
            <section>
              ...
              <div className="space-y-5">
                <div className="flex flex-col gap-1.5">
                  ...
                  <select value={activePage?.aspectRatio} onChange={(e) => handleCanvasSizeChange(e.target.value as AspectRatio)} className="bg-zinc-800 border border-zinc-700 rounded px-3 h-10 text-xs text-white focus:outline-none focus:border-blue-500 cursor-pointer hover:bg-zinc-750 transition-colors">
                    <optgroup label="Digital & Social" className="bg-zinc-900 text-zinc-400">
                      {['1:1', '2:3', '3:2', '3:4', '4:3', '4:5', '5:4', '9:16', '16:9'].map(ratio => (
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-zinc-500 font-semibold">Background</label>
                  <div className="flex items-center gap-3">
                    <input
                      type="color"
                      value={activePage?.backgroundColor || '#ffffff'}
                      onChange={(e) => onUpdatePage({ backgroundColor: e.target.value })}
                      className="h-12 w-16 bg-transparent border-0 rounded-lg cursor-pointer p-0"
                    />
                    <input
                      type="text"
                      value={activePage?.backgroundColor || '#ffffff'}
                      onChange={(e) => onUpdatePage({ backgroundColor: e.target.value })}
                      className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-3 h-10 text-[10px] text-white font-mono uppercase"
                    />
                  </div>
                </div>
              </div>
