...
export const updateAdminUser = async (id, updates) => {
  ...
  if (updates.reset_trial === true) {
    const now = new Date();
    const nextExpiry = new Date(now.getTime() + 24 * 60 * 60 * 1000);
      ...
      .upsert(
        {
          ...
          motion_downloads_used: 0,
        },
        { onConflict: 'app_user_id' }
      );

    if (trialUpsertError) throw new Error(trialUpsertError.message);
  }
};
