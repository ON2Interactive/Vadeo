...
export const STANDARD_MOTION_AI_LIMIT = 5;
export const PREMIUM_MOTION_AI_LIMIT = 10;
...
export const getTrialStateFromRow = (row) => {
  ...
  const motionDownloadsUsed = row?.motion_downloads_used || 0;

  if (!startedAt || !expiresAt) {
    return { status: 'none', startedAt: null, expiresAt: null, motionDownloadsUsed };
  }
...
export const getMotionAiLimit = (plan) => {
  if (plan === 'premium') return PREMIUM_MOTION_AI_LIMIT;
  if (plan === 'standard') return STANDARD_MOTION_AI_LIMIT;
  return 0;
};

export const getRemainingMotionAi = (plan, used) => {
  const limit = getMotionAiLimit(plan);
  if (!limit) return 0;
